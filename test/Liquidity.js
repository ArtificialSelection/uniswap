...
before(async () => {
  ...
  await usdc.connect(usdcWhale).transfer(account[0].address, usdcAmount);
});
...
it("mintNewPosition", async () => {
  ...
  await dai.connect(account[0]).transfer(liquidityExamples.address, daiAmount);
  await usdc
    .connect(accounts[0])
  ...
  console.log(
    ...
    await dai.balanceOf(account[0], address)
  );
  console.log(
